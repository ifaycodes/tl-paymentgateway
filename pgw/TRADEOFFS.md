...
- **GatewayApis** — exposes REST endpoints to FicMart and orchestrates the flow
- **BankClient** — handles all HTTP communication with the Mock Bank API
- **PaymentRepository** — stores core payment data (paymentRef, authorizationId, customerId, orderId, amount, state)
...
## State Management
Payment state is tracked using a `State` enum (AUTHORIZED, CAPTURED, VOIDED, REFUNDED) with direct database updates on each operation. 
...
## Failure Handling
The gateway implements a retry strategy for bank calls that fail with transient errors (5xx responses). 
...
- **State changes** — I am currently manually updating the state of the each operation. I would have a more robust way to move this update. I have a state machine set up but I am having a bit error throw when i try to use it, still updating that
- **Cast a wider error net** — to be able to catch more instances that could go wrong
...
So I used SpringBoot Test, which I think is not doing a good. I'm not. The tests return well but I thought I don't actually get to really rough it up since i already know what it would return